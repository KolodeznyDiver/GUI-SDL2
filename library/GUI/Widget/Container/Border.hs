    ...
    mkWidget borderFlags
        ...
        onDraw= \widget -> do
            ...
            case borderType of
              ...
              Border3D{..} -> bkFill >> draw3DBorder (fromMaybe (brdr3DLightColor skin) border3DLightColor)
...
--            liftIO $ putStrLn $ concat ["label.onDraw getVisibleRect=", rectToBriefStr r]
            whenIsJust mbFnt $ \fnt ->
                drawStrAligned fnt (hvAlignToAlignment borderCaptionAlignment VTop)
                    frgrndColor (if isTransp then DrawStrFine else DrawStrOpaque bkgrndColor)
                    (shrinkRect (V2 CaptionPaddingX 0) r) $ T.unpack borderCaption
        ,onResizing= \widget newRect -> do
                r <- extendableOnResizing borderSize widget newRect
                let r' = shrinkRect' borderThickness r
